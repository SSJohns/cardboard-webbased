...
//ball.material = THREE.DoubleSide
...
//squareMesh[j].position.x = 15*j-15*text.length/2;
//squareMesh[j].position.z = -150;
...
function render(dt) {
    var timer = new Date().getTime() * 0.0005;
    //var v = (.5 + .5 * Math.cos(timer));
    //var zoom = -500 * v + 500 * (1 - v);
    //zoom* Math.cos(camera.rotation.y);
    //camera.position.z = Math.floor(Math.sin(timer)*150);//zoom* Math.sin(camera.rotation.y);
    //for(k = 0; k < squareMesh.length; k++){
    //	squareMesh[k].setRotationFromQuaternion(camera.quaternion);
    //}
    ...
    // if(camera.rotation.z > 1.5 && camera.rotation.x > 1.5){
    //     camera.rotation.x = 0;
    //     camera.rotation.x = 0;
    ...
    //         count = 0;
    //         trueFalse = true;
    //         init("textures/patterns/014_pano3.jpg");
    //         trueFalse = false;
    //     }else{
    //         while (scene.children.length > 0) {
    //             scene.remove(scene.children[scene.children.length - 1]);
